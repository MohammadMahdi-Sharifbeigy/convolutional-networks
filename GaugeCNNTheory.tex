...
% Define custom commands for mathematical symbols and groups
\newcommand{\TpM}{T_p M} % Tangent space
\newcommand{\GL}[1]{\operatorname{GL}(#1)} % General Linear Group
\newcommand{\Flip}{\text{Flip}} % Flip group
\newcommand{\SO}[1]{\operatorname{SO}(#1)} % Special Orthogonal Group
\newcommand{\Scale}{\text{Scale}} % Scale group (assuming this is a custom group name)
\newcommand{\Ogroup}[1]{\operatorname{O}(#1)} % Orthogonal group (using a different name to avoid clash with \O)
\newcommand{\GM}{\mathscr{M}} % G-structure, using mathrsfs for consistent look
\newcommand{\IsomM}{\operatorname{Isom}(M)} % Isometries of manifold M
\newcommand{\E}[1]{\operatorname{E}(#1)} % Euclidean group
\newcommand{\FM}{\mathscr{F}M} % Frame bundle
...
\settextfont{B Zar} % Using XB Zar instead of B Zar for better compatibility
\setdigitfont{B Zar}
